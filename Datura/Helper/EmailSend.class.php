 ...
 * 示例：
 * $mail=new EmailSend($data);
  $mail->setServer('smtp.163.com', 'lengfeng1601', 'hen0990123')->setFrom('lengfeng1601@163.com')->
  setReceiver('waitfox@qq.com')->setMail('欢迎测试', '汉语测试<b>测试发送</b>')->sendMail();
 */
class EmailSend {

    protected $_userName;
    protected $_password;
    protected $_sendServer;//邮件服务器的ip或者域名
    protected $_port;//代理服务器的端口，smtp默认25号端口
    protected $_from;
    protected $_to = array();
    protected $_cc = array(); //抄送
    protected $_bcc = array(); //密送
    protected $_subject;
    protected $_body;
    protected $_attachment = array();
    protected $_socket;
    protected $_isSecurity;//到服务器的连接是否为安全连接，默认false，gmail使用了SSL
    protected $_errorMessage;
    protected $debug; //调试模式，显示调试信息
    
    public function __construct(array $data) {
        $this->_sendServer = $data['Server'];
        $this->_port = $data['port'];
        $this->_isSecurity = $data['isSecurity'];
        $this->_userName =base64_encode($data['AuthUsername']);
        $this->_password = base64_encode($data['AuthPassword']);
